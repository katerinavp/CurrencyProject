...
class CurrencyViewModel @Inject constructor(private val repo: CurrencyRepository, private  val repoFavorites: FavoritesRepository) : ViewModel() {
    ...
    fun saveFavorites(currency: CurrencyDomainModel){
        viewModelScope.launch(Dispatchers.IO) {
            try {
                repoFavorites.insertFavorites(currency)
